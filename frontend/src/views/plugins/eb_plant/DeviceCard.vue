...
const props = defineProps({
  // 来自 layout 的设备绑定：{ device_id, device_name, tag, commands(schema) }
  binding: { type: Object, required: true },
  // 来自实时流的设备状态：{ status, is_online, last_seen, ts } 或 null
...
// 在线判断：优先按 last_seen/ts 现算（180s ≈ 3×默认心跳），无时间戳则回退后端 is_online
...
// 把任意 status dict 自动展开成行；布尔单独标记类型用于渲染开/关 pill
const statusRows = computed(() => {
  ...
  return Object.entries(st).map(([key, value]) => {
    ...
    if (typeof value === 'number') {
      return { key, type: 'num', value, display: fmtNumber(value) }
    }
    if (value !== null && typeof value === 'object') {
      return { key, type: 'str', value, display: JSON.stringify(value) }
    }
