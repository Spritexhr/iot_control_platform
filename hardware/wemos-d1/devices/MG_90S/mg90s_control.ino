 ...
 * 接线说明：
 * - 舵机信号线（橙色） → D5 (GPIO14)
 * - 舵机电源线（红色） → 5V（由于金属齿轮舵机电流较大，高负载下建议使用外部 5V 独立供电，并与 Wemos D1 共地）
 * - 舵机地线（褐色/黑色） → GND
 */
...
// ============ 舵机配置 ============
#define SERVO_PIN D5   // 舵机信号线接 D5 (GPIO14)

// 设备标识（请为每个设备设置唯一ID，需与 Django Admin 中创建的设备一致）
const char* DEVICE_ID = "mg90s_001";
...
void setupSerial() {
  ...
  delay(1000);
  Serial.println(F("\n========================================"));
  Serial.println(F("  MG90S 金属舵机控制 - 执行器设备"));
  Serial.println(F("========================================"));
  Serial.print(F("设备ID: "));
  Serial.println(DEVICE_ID);
  Serial.println(F("舵机引脚: D5 (GPIO14)"));
}

void setupServo() {
  myServo.attach(SERVO_PIN);
  myServo.write(currentAngle);  // 初始角度 90°
  Serial.println(F("✓ MG90S 金属舵机已初始化，初始角度 90°"));
}
