...
创建 `control` 视图后，“控制方案”页可以管理双位、PI、PID 控制方案：
...
“脚本规则”页复用平台自由 Python 自动化引擎。规则只能选择当前房间已经导入的传感器和设备，不能手工填写房间外资源 ID。超级用户可在视图内完成资源关联、脚本编辑和保存；工作人员可单次测试、启动或停止后台轮询。

项目脚本也会出现在全局“自动化规则”列表，并标注所属项目和房间。从全局详情页进入时仍使用相同的房间资源白名单。资源从房间移除后，规则不能继续执行；后台轮询会在下一拍进入错误停止状态。
