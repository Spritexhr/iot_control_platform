...
### 8.1 后台调度器 (Scheduler)
在平台中，当您点击“启动轮询”时，该规则会被打上 `is_launched=True` 的标记。后端的守护线程（Scheduler）会每秒钟在后台扫描这些规则，并根据设定的 `poll_interval`（轮询间隔）自动执行您的 `loop()` 方法。

这意味着：
- **您不需要在代码里写 `time.sleep()`**，请保持 `loop()` 方法足够轻量，执行完一次立即返回。
- 即使关闭了浏览器页面，脚本依然会在服务器后台不知疲倦地运行。

### 8.2 日志与调试
- **单次执行**：在前端页面点击“单次执行”，可以将代码中的 `print()` 输出以及发生错误时的报错信息直接显示在网页的终端中。
- **后台日志**：后台轮询过程中的正常执行日志被记录在 `logs/automation.log` 中。
- **后台崩溃保护**：如果您的 `loop()` 代码在后台运行时抛出了未捕获的异常（比如除以零、调用了不存在的方法等），后台调度器会捕获这个异常，将该规则的状态改为“报错终止” (`error_stopped`)，停止对它的轮询调用，并将报错堆栈显示在前端的卡片上，防止一个错误的脚本把整个系统拖垮。
