...
| `frontend`  | Vue 前端构建 + Nginx 提供静态文件与 API 代理 | 80（可改为 8080） |
| `backend`   | Django REST API + Gunicorn   | 内部 8000（不直接对外） |
| 数据卷       | 持久化 MySQL 数据            | -        |
...
数据库迁移会在后端启动时自动执行。首次部署只需创建管理员账号：
...
- `frontend`：构建前端，映射端口，依赖 `backend`
- `backend`：构建后端，挂载数据卷持久化 SQLite
- `volumes`：`backend_data` 保存 `db.sqlite3`
...
若需使用 MySQL 替代 SQLite：

1. 在 `docker-compose.yml` 中添加 `mysql` 服务
2. 修改 `config/settings.py` 或通过环境变量支持 MySQL 配置
3. 将 `backend` 的 `depends_on` 设为 `mysql`
4. 在 `.env` 中配置 `DB_HOST=mysql` 等
...
**原因**：本地开发和 Docker 用的是**两个完全独立的数据库文件**：

| 运行方式 | 数据库位置 |
|---------|------------|
| 本地开发 (`python manage.py runserver`) | `iot_control_platform/db.sqlite3` |
| Docker 运行 | Docker 数据卷 `backend_data` → 容器内 `/app/data/db.sqlite3` |

Docker 容器不会读取你本地的 `db.sqlite3`，所以看起来是"全新的"。

**如何让 Docker 使用本地已有的数据库？**

在 `docker-compose.yml` 的 backend 服务中，修改 `volumes`，用本地文件挂载替代数据卷：

```yaml
volumes:
  - backend_data:/app/data   # 默认：使用 Docker 数据卷（全新库）
  # 改用本地数据库时，注释上一行，改用：
  # - ./iot_control_platform/db.sqlite3:/app/data/db.sqlite3
```

注意：使用本地挂载时，需确保 `iot_control_platform` 目录下存在 `db.sqlite3`，且容器有可写权限。首次可先创建 `data` 目录并复制数据库：

```bash
mkdir -p iot_control_platform/data
copy iot_control_platform\db.sqlite3 iot_control_platform\data\db.sqlite3
```

然后在 docker-compose 中挂载 `./iot_control_platform/data:/app/data`。
...
**原因**：Docker 容器内的 `localhost` 指向容器自身，无法访问宿主机上的服务。

**解决**：
- **EMQX 在本机运行**：在 `.env` 中设置 `MQTT_BROKER=host.docker.internal`（Windows/Mac Docker Desktop 支持）
- **EMQX 在远程服务器**：`MQTT_BROKER=实际IP或域名`
